...
import { FaFacebookF, FaTwitter, FaPinterestP, FaEnvelope } from 'react-icons/fa';
...
const ViewMenuItem: React.FC = () => {
  ...
  if (!restaurantId || !menuItemId) {
    return <div className="text-center py-8 text-red-500">Invalid restaurant or menu item ID.</div>;
  }
  ...
  return (
    <div className="w-full bg-white min-h-screen">
      ...
      <div className="container mx-auto px-4 py-4 text-sm text-gray-600">
        ...
        <Link to={`/restaurant/${restaurantId}/menu`} className="hover:underline">
      ...
      <div className="container mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/2 space-y-4">
          ...
          <p className="text-xl font-semibold text-gray-900">Rs. {menuItem.price.toFixed(2)}</p>
          <p className="text-gray-600">{menuItem.description || 'No description available.'}</p>
          ...
          <p className="text-gray-600">
            Category:{' '}
            <Link to={`/restaurant/${restaurantId}/menu`} className="text-orange-600 hover:font-bold">
              {menuItem.category || 'Menu'}
            </Link>
          </p>
          ...
          <div className="flex items-center gap-5">
            ...
            <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 p-2 bg-gray-50 rounded-2xl">
              <FaFacebookF />
            </a>
            <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-600 p-2 bg-gray-50  rounded-2xl">
              <FaTwitter />
            </a>
            <a href="https://pinterest.com" target="_blank" rel="noopener noreferrer" className="text-red-500 hover:text-red-700 p-2 bg-gray-50  rounded-2xl">
              <FaPinterestP />
            </a>
            <a href="mailto:example@example.com" className="text-gray-500 hover:text-gray-700 bg-gray-50  p-2 rounded-2xl">
              <FaEnvelope />
            </a>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8 ">
        <div className="border-t border-gray-300 pt-4">
          <div className="flex gap-4 mb-4">
            <button className="text-gray-800 font-semibold border-b-2 border-amber-600 pb-2">Description</button>
          </div>
          <div className="text-gray-600 space-y-2">
            <p>{menuItem.description || 'No description available.'}</p>
            {menuItem.size && menuItem.pieces && (
              <p>Size: {menuItem.size} / {menuItem.pieces} Pieces</p>
            )}
            {menuItem.orderHours && <p>Orders accepted from {menuItem.orderHours}</p>}
          </div>
        </div>
      </div>
